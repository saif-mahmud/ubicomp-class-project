...
def lp_vis(audio_file, audio_config, cutof_time, cutoff_freq=15000):
    ...
    for c in coi:
        print('File %s Channel %d: signal max: %.1f, after clapping: %.1f, mean: %.1f' % (os.path.basename(audio_file), c, np.max(np.abs(raw_audio[:, c])), np.max(np.abs(raw_audio[(syncing_pos + 5) * frame_length:, c])), np.mean(np.abs(raw_audio[:, c]))))
