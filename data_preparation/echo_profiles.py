...
def echo_profiles(audio_file, audio_config, target_bitwidth, maxval, maxdiff, mindiff, rectify=False, no_overlapp=False,
                  no_diff=False):
    ...
    start_profiles = np.zeros((n_tx, n_coi, min(frame_length * 100000,
                                                all_audio.shape[0] // (frame_length * n_tx * n_coi) * (
                                                        frame_length * n_tx * n_coi))))

    for n, tx in enumerate(tx_signals):
        for i, c in enumerate(audio_config['channels_of_interest']):
            filtered_audio[n, i] = butter_bandpass_filter(all_audio[:, c], bp_ranges[n][0], bp_ranges[n][1],
    ...
    # start_pos = (start_pos + frame_length) % frame_length # echo profile starting from the top
    start_pos = (start_pos + frame_length - frame_length // 2) % frame_length # echo profile starting in the middle
    print('Detected start pos: %d' % start_pos)

    # return

    filtered_audio = filtered_audio[:, :, start_pos:]
    filtered_audio = filtered_audio[:, :, :filtered_audio.shape[2] - filtered_audio.shape[
        2] % frame_length]  # c x seq_len = c x (n_frames x frame_length)
    filtered_audio = np.reshape(filtered_audio, (
        filtered_audio.shape[0], filtered_audio.shape[1], -1, frame_length))  # c x n_frames x frame_length
        ...
        for n, tx in enumerate(tx_signals):
            for c in range(n_coi):
                if audio_config['signal'].lower() == 'gsm':
                    ...
                    for f in range(profiles.shape[1]):
                        profiles[n, c, f, :] = np.correlate(filtered_audio[n, c, f, :], tx, mode='full')[
                                               frame_length - 1:]
    else:
        no_overlapp_text = ''
        for n, tx in enumerate(tx_signals):
    ...
    cv2.imwrite('%s_%s%s_%dbit_profiles.png' % (
        audio_file[:-4], audio_config['signal'].lower(), no_overlapp_text, target_bitwidth), profiles_img)
    np.save('%s_%s%s_%dbit_profiles.npy' % (
        audio_file[:-4], audio_config['signal'].lower(), no_overlapp_text, target_bitwidth), profiles)
    if not no_diff:
        cv2.imwrite('%s_%s%s_%dbit_diff_profiles.png' % (
            audio_file[:-4], audio_config['signal'].lower(), no_overlapp_text, target_bitwidth), diff_profiles_img)
        np.save('%s_%s%s_%dbit_diff_profiles.npy' % (
            audio_file[:-4], audio_config['signal'].lower(), no_overlapp_text, target_bitwidth), diff_profiles)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sample_depth', help='sampling depth (bit) of audio file, comma-separated', type=int,
                        default=0)
    ...
    echo_profiles(args.audio, audio_config, args.target_bitwidth, args.maxval, args.maxdiffval, args.mindiffval,
                  args.rectify, args.no_overlapp, args.no_diff)
