...
class DefaultPredictor_Lazy:
    ...
    def __init__(self, cfg, gpu_idx=0):
        ...
        if isinstance(cfg, CfgNode):
            ...
            checkpointer = DetectionCheckpointer(self.model)
            checkpointer.load(OmegaConf.select(
                cfg, "train.init_checkpoint", default=""))

            mapper = instantiate(cfg.dataloader.test.mapper)
            self.aug = mapper.augmentations
            self.input_format = mapper.image_format

        if gpu_idx == -1 or not torch.cuda.is_available():
            device = torch.device('cpu')
        else:
            device = torch.device(f'cuda:{gpu_idx}')

        print(f'Device for Detectron2 Model: {device}')
    ...
    def __call__(self, original_image):
        ...
        with torch.no_grad():
            ...
            image = self.aug(T.AugInput(original_image)
